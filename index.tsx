...
const getHexagramFromLines = (lines: ('yang' | 'yin')[]): HexagramData | undefined => {
    ...
    const found = HEXAGRAM_DATA.find(h => {
        // This is a simplification. A real mapping would be more complex.
        // For now, we find by matching trigram names in the hexagram name.
        const name = h.vietnameseName.toLowerCase();
        const lowerStr = lowerTrigram.toLowerCase().replace('thuần ', '');
        const upperStr = upperTrigram.toLowerCase().replace('thuần ', '');
        
        // This logic is a placeholder and needs a proper trigram-to-hexagram mapping matrix
        // For demo purposes, we will use a simple lookup based on names which might not be fully accurate.
        // A proper implementation would use a matrix like `King Wen sequence`.
        // Let's find one based on a known example: Thủy Lôi Truân (Khảm over Chấn)
        if (h.vietnameseName === "Thủy Lôi Truân" && upperTrigram === "Khảm" && lowerTrigram === "Chấn") return true;
        
        // This is a simplified direct lookup based on the final hexagram name, which we will determine by another means
        // The most reliable way is to have a complete map. Let's create one.
        return false;
    });

    // A more reliable mapping is needed. For now, let's hardcode a few and then find a better system.
    // Let's create a full map.
    ...
    useEffect(() => {
        const cast = () => {
            ...
            const interval = setInterval(() => {
                ...
                if (method === 'Cỏ Thi') {
                    setStepMessage(`Đang tính hào ${currentLines.length + 1}/6 bằng Cỏ Thi...`);
                    ...
                    setStepMessage(`Đang tính hào ${currentLines.length + 1}/6 bằng Mai Hoa...`);
    ...
    return (
        <div className="result-screen">
            ...
            <div className="ai-interpretation">
                {isLoading && <div className="spinner" aria-label="Đang tải luận giải"></div>}
